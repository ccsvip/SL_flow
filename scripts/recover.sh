...
#   * The "绔嬪嵆搴旂敤鏇存柊" button left the stack in a bad state (502, restarting
#     loop, or stuck "another update in progress").
#   * You manually changed code on disk and want to safely apply it.
#
# Modes:
#   bash scripts/recover.sh              # full rebuild + recovery (default)
#   bash scripts/recover.sh --diagnose   # READ-ONLY: dump logs only, do not
#                                        # touch any container. Use this first
#                                        # when you want to know WHY the stack
#                                        # broke before rebuilding.
#
# What the default mode does, in order:
#
...
# --- Diagnose mode: read-only dump for figuring out why the stack broke,
#     without changing anything. Always exits 0; the operator reads stderr/
#     stdout and pastes it to whoever is helping.
if [[ "${1:-}" == "--diagnose" || "${1:-}" == "-d" ]]; then
    log "DIAGNOSE MODE - read only, no containers will be touched"
    log "Workspace: $REPO_DIR"

    log "Git state"
    (git rev-parse --abbrev-ref HEAD; git log -1 --oneline; git status --porcelain) 2>&1 || true

    log "Lock + updater state"
    if [[ -f "$REPO_DIR/.update.lock" ]]; then
        echo "lock exists, mtime=$(stat -c '%y' "$REPO_DIR/.update.lock" 2>/dev/null || echo unknown)"
    else
        echo "no .update.lock"
    fi
    docker ps -a --filter name=slflow-updater --format 'updater: {{.Names}} status={{.Status}} created={{.CreatedAt}}' || true

    log "Compose state"
    docker compose ps 2>&1 || true

    log "Backend logs (last 100)"
    docker compose logs backend --tail=100 2>&1 || true

    log "Frontend logs (last 40)"
    docker compose logs frontend --tail=40 2>&1 || true

    log "Hot-update log tail (last 100)"
    tail -n 100 "$REPO_DIR/.last_update.log" 2>/dev/null || echo "(no .last_update.log)"

    log "Disk space"
    df -h "$REPO_DIR" 2>&1 || true

    log "Diagnose complete - paste this output to whoever is helping debug."
    exit 0
fi
...
# 2. Pre-pull the hot-update sibling image so future "绔嬪嵆搴旂敤鏇存柊" clicks
